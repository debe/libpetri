...
**Minting** (NU-010). An action mints a fresh name through the context (`ctx.freshName()`). Names are unique across all firings of one execution and replay-stable for a fixed firing order **and a fixed scope** (see below). The minter qualifies names by the firing transition's post-composition, instance-prefixed name, which means **freshness is per instance for free** (NU-030): two instances of the same forking subnet draw from disjoint pools with no extra runtime state.

**Minting across a resume** (NU-011; Java and TypeScript 6.1, Rust 7.0, Python 6.0). A restore starts a *new* execution, and a join correlates on name equality alone, so an executor that re-minted a name already in its restored marking would silently merge a restored token with an unrelated fresh one. Minted names are therefore `<transition>#<scope>:<n>`, where `<n>` is a per-executor counter from 0 and the scope belongs to the execution. The default scope is a random 32-hex token, unique across processes, so a resume is collision-safe with no host action. It is not the execution id, and it is not reproducible.

Pin `executionScope` (`execution_scope` in Rust and Python) only when a replay must reproduce the exact names, and then give **every segment of a restore lineage its own scope**: resuming under the scope the snapshot was minted under is the collision again, and nothing detects it. A scope must be non-empty and contain neither `:` nor `#`; with those banned a name parses uniquely (last `:` splits the counter, then the last `#` before it splits the scope). Never build a design on the *shape* of a minted name: project identity out of the payload, as always. Tests compare names structurally or under a pinned scope, never against a literal from a default scope.

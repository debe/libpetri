...
7. **There is no optional output.** An unconditional `And` for something the action sometimes cannot produce is a runtime violation and a deadlock in the model. Write `xor(realOutput, voidSink)`.

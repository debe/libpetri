...
pub fn build(name: &str) -> FixtureNet {
    match name {
        ...
        // that used to separate the two routes' deadlock predicates. [VER-002]
        // now mandates Route B's strict reading for DeadlockFree and gives the
        // old permissive one its own property, TerminatesAtSink, so both routes
        // decide the same predicate ([VER-002] AC7) and a disagreement here is a
        // parity FINDING, not documentation.
        ...
        // non-sink `stuck`. Route B: violated, and Route A agrees on the same
        // shape (see `sinkPartialTerminal`, also violated). Non-vacuity guard: a failed ν
        ...
        // {branchA:1, branchB:1} and turn this fixture violated. Route A agrees
        // on the same shape (see `sinkDrainedTerminal`, also proven).
        "nuDrainedTerminal" => {
            let source = Place::<()>::new("source");
            let a = Place::<String>::new("branchA");
            let b = Place::<String>::new("branchB");

...
- **Verification (NU-050):** the SMT verifier is ν-aware. New `BranchPlaceBound` / `JoinedOrDeadLettered` properties; a budget-place declaration (`budgetPlaces(...)` / `budget_places=`) asserts the bounded fragment. The sound baseline (all bindings) over-approximates matched transitions name-blind — sound for `Proven` on reachability-safety bounds — so quiescence properties on a ν-net and any ν-net without a declared budget return `Unknown` (mirroring VER-006). All four bindings additionally decide the bounded mint→matched-join fragment *exactly* via bounded name-colouring (Route A, NU-050 #1) — no spurious different-name counterexample.

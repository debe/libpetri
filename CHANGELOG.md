...
- **ν verification: Route B no longer proves what it cannot see (all languages, [VER-006] AC8).** Under `alwaysAvailable` or `bounded(k)` the name-partition state-class graph keeps an environment place at its initial count, with no injected names. A property naming an environment place, an inhibitor on one, a join keyed on one, or conflict priority on a shared environment input could come back vacuously `Proven`; Route B now returns `Unknown` with the reason, and its quiescence verdicts carry the [VER-006] AC6 vacuity note. Rust state-class graphs now gate read arcs on environment places, and `Bounded(k)` demand (`required ≤ k`), as Java and TypeScript do.

...
* `occupied` / `free` — RB2: live positions hold `Some`, free positions of the
  *current* block hold `None`. This is what makes `ring_remove_first`'s
  `.take().unwrap()` (`precompiled_backend.rs:389`) total. Slots outside every
...
/-! ## `ring_remove_first` (`precompiled_backend.rs:386-393`)
...
/-- Totality of the `.unwrap()` at `precompiled_backend.rs:389`: under RB2 a
...
/-! ## `ring_peek_first` (`precompiled_backend.rs:416-421`) -/
...
/-! ## `grow_ring_static` (`precompiled_backend.rs:1377-1404`)
...
/-! ## `ring_add_last` (`precompiled_backend.rs:396-413`)
...
/-! ## `ring_remove_matching` (`precompiled_backend.rs:428-480`)
...
branch, `precompiled_backend.rs:452-460`) -/
...
branch, `precompiled_backend.rs:461-474`) -/

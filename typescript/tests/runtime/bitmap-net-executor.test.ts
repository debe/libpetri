...
describe('Failure Containment (E1) + Timeout Output Merge (E2)', () => {
  ...
  it('E2-xor: a pre-timeout write does not merge with the timeout branch', async () => {
    ...
    const net = PetriNet.builder('N').transition(t).build();
    const eventStore = new InMemoryEventStore();
    const executor = new BitmapNetExecutor(net, initialTokens([input, [tokenOf('go')]]), { eventStore });
    const marking = await executor.run(5000);
...
describe('Event Store Tests', () => {
  ...
  it('noop event store has no allocations', async () => {
    ...
    const executor = new BitmapNetExecutor(net, initialTokens([input, [tokenOf('go')]]), { eventStore: noop });
...
describe('Intermediate Marking Clock Restart Tests', { timeout: 15_000 }, () => {
  ...
  async function runWithActivity(
    ...
    const executor = new BitmapNetExecutor(net, tokens, {
      eventStore,
...
describe('Deadline Enforcement Tests', () => {
  ...
  it('rejects a negative deadlineToleranceMs', () => {
    ...
    const t = Transition.builder('T')
      ...
      .build();
    const net = PetriNet.builder('N').transition(t).build();
    expect(() => new BitmapNetExecutor(net, new Map(), { deadlineToleranceMs: -1 })).toThrow();
  });
});

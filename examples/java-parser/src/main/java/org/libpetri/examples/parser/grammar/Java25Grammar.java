...
public final class Java25Grammar {
    ...
    public static Grammar create() {
        ...
        // ==================== Lexical (§3) ====================
        prods.add(prod("Identifier", tokenMatch(TokenType.IDENTIFIER)));
        ...
        prods.add(prod("AnnotationArgs", choice(
            // name = value pairs or single value
            seq(nonTerminal("Identifier"), terminal("="), nonTerminal("ElementValue"),
                repeat(seq(terminal(","), nonTerminal("Identifier"), terminal("="), nonTerminal("ElementValue")))),
        ...
        prods.add(prod("ElementValueArrayInitializer", seq(
            terminal("{"),
            optional(seq(nonTerminal("ElementValue"),
        ...
        prods.add(prod("FieldOrMethodOrConstructor", choice(
            ...
            seq(optional(nonTerminal("TypeParameters")),
                nonTerminal("Identifier"), nonTerminal("IdentBasedMemberRest"))
        )));
        ...
        prods.add(prod("AnnotationDeclRest", seq(
            terminal("@"), terminal("interface"), nonTerminal("TypeIdentifier"),
            terminal("{"),
            repeat(nonTerminal("InterfaceBodyDeclaration")),
            terminal("}")
        )));
        ...
        prods.add(prod("LabelOrExprRest", choice(
            ...
            terminal(";"),
            // Class-type local variable declaration: Type was parsed as Expression,
            // and IDENTIFIER follows (the variable name).
            // E.g. "Runnable r = () -> {};" or "String s = hello;"
            seq(nonTerminal("Identifier"),
                optional(nonTerminal("Dims")),
                optional(seq(terminal("="), nonTerminal("VariableInitializer"))),
                repeat(seq(terminal(","), nonTerminal("VariableDeclarator"))),
                terminal(";"))
        ...
        prods.add(prod("RelationalSuffix", choice(
            seq(choice(terminal("<"), terminal("<="), terminal(">=")),
                nonTerminal("ShiftExpression")),
            // '>' is ambiguous with generics close, handle carefully
            seq(terminal(">"), nonTerminal("ShiftExpression")),
        ...
        // Shift expression: only left-shift is handled here since >> and >>> conflict
        // with relational > and generic type closing >.
        // Right-shift (>> and >>>) is not supported to avoid LL(1) conflicts.
        ...
        // Inside parens: expression (paren expr) or empty (lambda with no params).
        prods.add(prod("ParenContent", optional(nonTerminal("Expression"))));
        ...
        prods.add(prod("NameSuffix", choice(
            ...
            // Array access: [expr]
            seq(terminal("["), nonTerminal("Expression"), terminal("]")),
        ...
        prods.add(prod("ClassCreatorRest", seq(
            optional(nonTerminal("TypeArguments")),
            nonTerminal("ClassOrInterfaceType"),
            optional(nonTerminal("TypeArgumentsOrDiamond")),
            terminal("("), optional(nonTerminal("ArgumentList")), terminal(")"),
            optional(nonTerminal("ClassBody"))
        )));

        prods.add(prod("TypeArgumentsOrDiamond", choice(
            nonTerminal("TypeArguments"),
            seq(terminal("<"), terminal(">"))
        )));

        prods.add(prod("ArrayCreatorRest", choice(
            seq(nonTerminal("Type"), nonTerminal("DimExprs"), optional(nonTerminal("Dims"))),
            seq(nonTerminal("Type"), nonTerminal("Dims"), nonTerminal("ArrayInitializer"))
        )));

        prods.add(prod("DimExprs", seq(
            terminal("["), nonTerminal("Expression"), terminal("]"),
            repeat(seq(terminal("["), nonTerminal("Expression"), terminal("]")))
        )));

...
public final class GrammarNetCompiler {
    ...
    private CompiledParserNet doCompile() {
        ...
        // Connect start production end → global end, wrapping AST into root node
        var startProdEnd = productionEndPlaces.get(grammar.startProduction());
        var globalExitTransition = Transition.builder("global_exit")
            .inputs(Arc.In.one(startProdEnd))
            .outputs(Arc.Out.place(globalEnd))
            .action(ctx -> {
                var state = ctx.input(startProdEnd);
                // Collect all remaining AST nodes into a CompilationUnit root
                var children = new ArrayList<AstNode>();
                var frame = state.astStack();
                while (frame != null) {
                    children.add(frame.peek());
                    frame = frame.pop();
                }
                Collections.reverse(children);
                var root = new AstNode.GenericNode("CompilationUnit", children);
                var finalState = new ParseState(state.tokens(), state.position(), state.callStack(),
                    AstFrame.of(root, null));
                emit(ctx, globalEnd, finalState);
                return CompletableFuture.completedFuture(null);
            })
            .build();
        allTransitions.add(globalExitTransition);
    ...
    private SubNet compileNonTerminal(GrammarElement.NonTerminal nt, String context) {
        ...
        var callTransition = Transition.builder(context + "_call_" + nt.name())
            ...
            .action(ctx -> {
                var state = ctx.input(start);
                emit(ctx, prodStart, state.pushCall(siteId));
                return CompletableFuture.completedFuture(null);
            })
            .build();
            ...
            .action(ctx -> {
                ...
                for (int i = 0; i < branchPredicates.size(); i++) {
                    if (branchPredicates.get(i).test(current)) {
                        emit(ctx, branchSubnets.get(i).startPlace(), state);
                        return CompletableFuture.completedFuture(null);
                    }
                }
        ...
        var decide = Transition.builder(context + "_rep_decide")
            ...
            .action(ctx -> {
                ...
                if (bodyPredicate.test(state.current())) {
                    emit(ctx, bodySubnet.startPlace(), state);
                } else {
                    emit(ctx, end, state);
                }
                return CompletableFuture.completedFuture(null);
            ...
            if (sites.size() == 1) {
                ...
                var returnTransition = Transition.builder(prodName + "_return")
                    ...
                    .action(ctx -> {
                        ...
                        emit(ctx, site.returnPlace(), state.reduceAst(prodName).popCall());
                        return CompletableFuture.completedFuture(null);
                    })
                    .build();
                ...
                var returnTransition = Transition.builder(prodName + "_return")
                    ...
                    .action(ctx -> {
                        ...
                        var reduced = state.reduceAst(prodName).popCall();

                        int index = (returnSiteId >= 0 && returnSiteId < siteIdToIndex.length)
                            ? siteIdToIndex[returnSiteId] : -1;

                        if (index >= 0) {
                            emitRaw(ctx, returnPlaces[index], reduced);
                        } else {
                            emit(ctx, errorPlace, reduced);
                        }
                        return CompletableFuture.completedFuture(null);
                    })
                    .build();
    ...
    @SuppressWarnings("unchecked")
    private static final Class<ParseState> PARSE_STATE_CLASS = (Class) ParseState.class;

    private Place<ParseState> makePlace(String name) {
        placeCounter++;
        return Place.of(name, PARSE_STATE_CLASS);
    }
    ...
    /**
     * Check if a token matches a terminal value.
     * For "&lt;" and "&gt;", also checks that the token is NOT TYPE_OPEN/TYPE_CLOSE
     * (those are matched by tokenMatch(TYPE_OPEN) / tokenMatch(TYPE_CLOSE) instead).
     */
    private static boolean matchesTerminal(LexToken current, String value) {
        if (!current.value().equals(value)) return false;
        // Prevent operator terminals from consuming type-argument tokens
        if (value.equals("<") && current.type() == TokenType.TYPE_OPEN) return false;
        if (value.equals(">") && current.type() == TokenType.TYPE_CLOSE) return false;
        return true;
    }

    /** Output a ParseState token with a fixed EPOCH timestamp, bypassing Instant.now(). */
    private static void emit(TransitionContext ctx, Place<ParseState> place, ParseState state) {
        ctx.output(place, new Token<>(state, ParseState.EPOCH));
    }

    /** Unchecked emit for raw Place<?> from XOR arrays. */
    @SuppressWarnings("unchecked")
    private static void emitRaw(TransitionContext ctx, Place<?> rawPlace, ParseState state) {
        Place<ParseState> place = (Place) rawPlace;
        ctx.output(place, new Token<>(state, ParseState.EPOCH));
    }
